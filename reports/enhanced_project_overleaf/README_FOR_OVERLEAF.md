...
This folder contains the final enhanced Overleaf report for the MLSS exoplanet habitability classifier. It uses the actual enhanced-pipeline results generated by the reproducible training workflow.
...
There should not be a single outer folder wrapping everything.
...
The report uses relative paths such as:
...
- enhanced model comparison results from `reports/model_comparison.csv`
- confusion matrices for all trained models and the DummyClassifier baseline
- ROC and precision-recall curves
- permutation importance for the selected best model
- historical notebook results clearly labeled as historical
- scientific caveats around the derived `habitable?` label

The headline enhanced result is that Logistic Elastic Net and SVM RBF tie for best test macro F1 at `0.9648`, while XGBoost has the best cross-validated macro F1 at `0.9559`.
